...
**Bloqué :** *[à compléter avec la durée réelle]* : générer le wrapper `mvnw` en Maven, puisque le squelette initial était en Gradle, et fixer les fins de ligne de `mvnw` avec `.gitattributes`. J'ai aussi dû réordonner deux issues : la #3 (liste des promotions) avant la #2, parce que l'écran d'ouverture de session a besoin de choisir une promotion.
